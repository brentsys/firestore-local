...
class DocumentPath {
    constructor(id, collectionPath) {
        ...
        if (collectionPath.length % 2 == 0)
            throw new Error("Collection Path should have odd length");
    }
...
class RecordModel {
        ...
        for (let elm of elements) {
            ...
            if (target !== undefined && target._seconds !== undefined) {
                if (target.toMillis === undefined) {
                    this[elm] = new firestore_1.Timestamp(target._seconds, target._nanoseconds);
                }
            }
        }
    }
    ...
    save(cfg, id) {
        var data = this.data();
        let customProps = utils_1.getCustomProperties(data);
    ...
    set(cfg, data) {
        return this.getDocumentRef(cfg).set(data, { merge: true });
    }
    validate(model) {
